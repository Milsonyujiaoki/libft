...
int main(void) {
    ...
    char validos[] = " \t"; // Espaço e tabulação são considerados "blank"
    
    // 2. Casos Inválidos Próximos (Devem falhar - Erros comuns de intervalo)
    char invalidos[] = "\n\r\f\v"; // Outros caracteres de espaço em branco não são "blank"
    ...
    for (size_t i = 0; invalidos[i] != '\0'; i++) {
        int res = ft_isblank(invalidos[i]);
        printf("Char '%c' (ASCII %3d) -> Retorno: %d [%s]\n", 
    ...
    for (size_t i = 0; i < sizeof(especiais) / sizeof(especiais[0]); i++) {
        int res = ft_isblank(especiais[i]);
