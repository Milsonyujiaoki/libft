...
int main(void) {
    ...
    char invalidos[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; // Letras maiúsculas e números
    ...
    for (size_t i = 0; invalidos[i] != '\0'; i++) {
        int res = ft_islower(invalidos[i]);
        printf("Char '%c' (ASCII %3d) -> Retorno: %d [%s]\n", 
    ...
    for (size_t i = 0; i < sizeof(especiais) / sizeof(especiais[0]); i++) {
        int res = ft_islower(especiais[i]);
